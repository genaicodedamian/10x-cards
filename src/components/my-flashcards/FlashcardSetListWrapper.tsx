...
const FlashcardSetListWrapper: React.FC = () => {
  ...
  const fetchFlashcardSets = useCallback(async (page: number) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Nie udało się załadować Twoich zestawów fiszek. Spróbuj ponownie później."
      );
  ...
  return (
    <div>
      ...
      {paginationInfo && paginationInfo.total_pages > 1 && (
        <PaginationControls paginationInfo={paginationInfo} onPageChange={handlePageChange} currentPage={currentPage} />
      )}
    </div>
  );
};
