...
const createFlashcardSetSchema = z.object({
  ...
  source_text_length: z.number().int()
    ...
    .optional(),
  generation_duration_ms: z.number().int()
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    if (serviceError) {
      ...
      return new Response(
        JSON.stringify({ message: 'Internal Server Error: Could not create flashcard set.' }),
    ...
    return new Response(
      JSON.stringify({ message: 'Internal Server Error: An unexpected issue occurred.' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
    ...
    return new Response(
      JSON.stringify({ message: 'Internal Server Error: An unexpected error occurred.' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}
